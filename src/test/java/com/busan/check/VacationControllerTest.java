...
public class VacationControllerTest {
	...
	void postVacation_whenVacationIsValid_vacationSavedWithUserInOf() {
		...
		Vacation vacation = TestUtil.createValidVacation();
		ResponseEntity<VacationVM> response = postVacation(vacation, user.getUsername(), VacationVM.class);
		assertThat(response.getBody().getUser().getUsername()).isEqualTo("홍길동");
	}
	@Test @DisplayName("휴가 등록시 user 객채도 같이 저장")
...
//		vacationService.save(TestUtil.createUsingVacation(), );
//		vacationService.save(TestUtil.createUsingVacation(), );
//		vacationService.save(TestUtil.createUsingVacation(), );
//		
//		ResponseEntity<TestPage<Object>> response = getVacations(new ParameterizedTypeReference<TestPage<Object>>() {});
...
//	@Test @DisplayName("존재하지 않는 회원의 군번으로 소속 병사 휴가자 조회시 status 404 받음")
...
//		IntStream.rangeClosed(1, 3).mapToObj(i -> i)
//			.map(TestUtil::createValidUser).forEach(userRepository::save);
//		vacationService.save(TestUtil.createUsingVacation(), 1);
//		vacationService.save(TestUtil.createUsingVacation(), 2);
//		vacationService.save(TestUtil.createUsingVacation(), 3);
//		
//		ResponseEntity<TestPage<Object>> response = getVacationsOfMyUnit(username, new ParameterizedTypeReference<TestPage<Object>>() {});
//		assertThat(response.getBody().getTotalElements()).isEqualTo(1);
//	}
